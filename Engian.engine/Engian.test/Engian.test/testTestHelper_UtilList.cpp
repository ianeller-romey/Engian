...
namespace Test
{
    ...
    template< typename T >
    bool const TestHelper_UtilList::CheckEmptyUtilList( Util::List< T > const& list )
    {
    ...
    template< typename T >
    bool const TestHelper_UtilList::CheckUtilListFrontAndBack( Util::List< T > const& list, unsigned const size, bool endCheck )
    {
    ...
    template< typename T >
    bool const TestHelper_UtilList::CheckUtilListAgainstArray( Util::List< T > const& list, 
    ...
    template< typename T >
    bool const TestHelper_UtilList::CheckUtilListAgainstContainer( Util::List< T > const& list, Util::Container< T > const& container )
    {
      unsigned i = 0;
      Util::List< T >::ListNode *node = list.m_front;
      ...
      for( ; node != 0 && itB != itE; node = node->m_next, ++itB, ++i )
      {
        ...
        if( node->m_next != list.m_end )
        {
          if( ( *( node->m_next->m_data ) == ( Util::Container< T >::Iterator( itB )++ ).Get() ) == false ) return "did not properly initialize a node's m_next pointer";
        }
        else
          if( ( i == TestHelper_UtilContainer::GetUtilContainerSize< T >( container ) - 1 ) == false ) return "did not properly initialize a node's m_next pointer, as an element that isn't the last one has an m_next pointer pointing to 0";
