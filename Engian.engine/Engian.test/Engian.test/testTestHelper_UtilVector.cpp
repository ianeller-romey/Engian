...
namespace Test
{
    ...
    unsigned const TestHelper_UtilVector::Vector_Clear()
    {
      ...
      RETURNLINEIFFAILED( vector.m_implementations == 0 );
      RETURNLINEIFFAILED( UnitTestSampleStruct::s_numberOfDeletes == number );      
    ...
    template< typename T >
    static char const * const TestHelper_UtilVector::CheckUtilVectorInit( Util::Vector< T > const& vector, 
                                                                          unsigned const capacity,
                                                                          unsigned const size )
    {
      if( vector.m_capacity != capacity ) return "did not initialize m_capacity correctly";
    ...
    static char const * const TestHelper_UtilVector::CheckUtilVectorInit( Util::Vector< T > const& vector, 
                                                                          unsigned const size )
    {
      if( vector.m_size != size ) return "did not increase m_size to the correct value";
      if( vector.m_array == 0 ) return "did not initialize m_array";
      if( vector.m_implementations != 0 ) return "created unnecessary Iterators";
      return 0;
    }

    
    template< typename T >
    static char const * const TestHelper_UtilVector::CheckUtilVectorAgainstArray( Util::Vector< T > const& vector, 
                                                                                  T const * const compareAgainst, 
                                                                                  unsigned const compareAgainstLen )
    {      
      for( unsigned i = 0; i < compareAgainstLen; ++i )
        if( vector.m_array[ i ] != compareAgainst[ i ] ) return "did not insert correct value";
      return 0;
    }
  
    
    template< typename T >
    static char const * const TestHelper_UtilVector::CheckUtilVectorAgainstContainer( Util::Vector< T > const& vector, 
                                                                                      Util::Container< T > const& container )
    {    
      Util::Container< T >::Iterator itB = container.Begin(),
                                     itE = container.End();
      for( unsigned i = 0; itB != itE; ++itB, ++i )
        if( vector.m_array[ i ] != itB.Get() ) return "did not insert correct value";
      return 0;
    }
