...
namespace Test
{
    ...
    template< typename T >
    static char const * const TestHelper_UtilUnsafeVector::CheckUtilUnsafeVectorInit( Util::UnsafeVector< T > const& uVector, 
                                                                                      unsigned const capacity,
                                                                                      unsigned const size )
    {
      if( uVector.m_capacity != capacity ) return "did not initialize m_capacity correctly";
    ...
    static char const * const TestHelper_UtilUnsafeVector::CheckUtilUnsafeVectorInit( Util::UnsafeVector< T > const& uVector, 
                                                                                      unsigned const size )
    {
      if( uVector.m_size != size ) return "did not increase m_size to the correct value";
      if( uVector.m_array == 0 ) return "did not initialize m_array";
      if( uVector.m_implementations != 0 ) return "created unnecessary Iterators";
      return 0;
    }

    
    template< typename T >
    static char const * const TestHelper_UtilUnsafeVector::CheckUtilUnsafeVectorAgainstArray( Util::UnsafeVector< T > const& uVector, 
                                                                                              T const * const compareAgainst, 
                                                                                              unsigned const compareAgainstLen )
    {      
      for( unsigned i = 0; i < compareAgainstLen; ++i )
        if( uVector.m_array[ i ] != compareAgainst[ i ] ) return "did not insert correct value";
      return 0;
    }
  
    
    template< typename T >
    static char const * const TestHelper_UtilUnsafeVector::CheckUtilUnsafeVectorAgainstContainer( Util::UnsafeVector< T > const& uVector, 
                                                                                                  Util::Container< T > const& container )
    {    
      Util::Container< T >::Iterator itB = container.Begin(),
                                     itE = container.End();
      for( unsigned i = 0; itB != itE; ++itB, ++i )
        if( uVector.m_array[ i ] != itB.Get() ) return "did not insert correct value";
      return 0;
    }
